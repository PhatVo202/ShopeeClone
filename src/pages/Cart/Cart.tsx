...
export default function Cart() {
  ...
  const choosenProductIdFromLocation = (location.state as { productId: string })?.productId
  console.log(choosenProductIdFromLocation)
  useEffect(() => {
    setExtendedProductDataCart((prev) => {
      const extendedPurchasesObject = keyBy(prev, '_id')
      return (
        productDataCart?.map((purchases) => {
          const isProductId = choosenProductIdFromLocation === purchases._id
          console.log(purchases._id)
          return {
            ...purchases,
            disabled: false,
            checked: isProductId || Boolean(extendedPurchasesObject[purchases._id]?.checked)
          }
        }) || []
      )
    }
    ...
    refetch()
  }, [productDataCart, choosenProductIdFromLocation])
          ...
          extendedProductDataCart.length > 0 ? (
            ...
            <div className="text-center ">
              ...
              <div className="mt-5 text-center">
                <Link to='/' className=" bg-orange px-10 hover:bg-orange/80 transition-all py-3 uppercase  text-white rounded-sm">
                  Mua ngay
                </Link>
              </div>
            </div>
